...
fun createPlayerAttributes(player: Player): List<ListItem> {
    ...
    items.add(ListItem("Nationality", player.nationality))
    ...
    player.statistics.forEach {
        ...
        if (!it.dribbles.toString().isNull()) {
    ...
    return items.filter { it.isHeader || (!it.isHeader && !it.value.isNull()) }
}